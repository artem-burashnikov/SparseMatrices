module SparseMatrices.MatrixAlgebra
...
let vecByMtx
    ...
    let powerSize = (max mtx.Rows mtx.Columns) |> ceilPowTwo
    let maxDepth = powTwo powerSize
    ...
    let rec inner parallelLevel bTree qTree depth : BinTree<'C> =
        ...
        let asyncCompute parallelLevel a1 a2 nw ne sw se depth =
            ...
            BinTree.Node(nodes[0], nodes[1]) |> SparseVector.reduce
        ...
        | BinTree.Node(a1, a2), QuadTree.Leaf _ ->
            ...
            BinTree.Node(result, result) |> SparseVector.reduce

        | BinTree.Node(a1, a2), QuadTree.Node(nw, ne, sw, se) ->
            if parallelLevel = 0u then
                BinTree.Node(fDo 0u a1 a2 nw sw depth, fDo 0u a1 a2 ne se depth)
                |> SparseVector.reduce
            else
    ...
    let mtxRowPower = mtx.Rows |> powTwo
    let mtxColumnPower = mtx.Columns |> powTwo
