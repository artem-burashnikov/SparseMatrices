module SparseMatrices.SparseMatrix
...
let tableToTree (table: Option<'A>[,]) =
    let rec maker (mtx: TableMatrix<'A>) =
        ...
        then
            let i = toIntConv mtx.HeadX
            let j = toIntConv mtx.HeadY
...
let cooMtxToTree (mtx: COOMatrix<'A>) =
    ...
    if mtx.Rows = 0u || mtx.Columns = 0u then
        ...
        let powerSize = ceilPowTwo (max mtx.Rows mtx.Columns)
        ...
        with get (i, j) =
            ...
            let getValue i j =
                ...
                if i >= this.Rows || j >= this.Columns then
                    ...
                    let powerSize = ceilPowTwo (max this.Rows this.Columns)
        ...
        let rec inner folder i j state size depth qTree =
            ...
            | QuadTree.Leaf value ->
                if depth = maxDepth then
                    ...
                    for m = toIntConv i to toIntConv mtx.Rows - 1 do
                        for n = toIntConv j to toIntConv mtx.Columns - 1 do
